 ...
 * ****
 * The fundamental methods are {@link #runFromMain(String[])} 
 * which runs the test class from its main method and 
 * {@link #runTestClass(String, Editor)} which runs a testclass with the given name. 
 *
 * @see GUIRunner
 * @see GUIRunListener
 * 
...
public final class Actions {
	...
	static class ExitAction extends AbstractAction {
		...
		ExitAction() {
			...
			putValue(SHORT_DESCRIPTION, "Quits this application immediately. ");
	...
	class CoreRunner extends Thread {
		...
		public void run() {
			...
			if (Actions.this.filter != null && Actions.this.filter.isTest()) {
				...
				Actions.this.listener
						.testClassStructureLoaded(request.getRunner().getDescription());
			}
	...
	/**
	 * Create the fundamental object to start, stop break tests and open new tests 
	 * monitored and controlled via a JUnit-GUI. 
	 *
	 * @param testClassName
	 *    the name of the test class to represent and run. 
	 * @param editor
	 *    the editor which is invoked for locating the source of a failure. 
	 */
	public Actions(String testClassName, Editor editor) {
		...
		this.guiRunner = new GUIRunner(this, editor);
	 ...
	 * @see #runFromMain(String[])
	 */
	static void runTestClass(final String testClassName, Editor editor) {
		Runnable guiCreator = new Runnable() {
			public void run() {
				// parameter null is nowhere used. 
				new Actions(testClassName, editor).startAction.actionPerformed(null);
			}
		};
	 ...
	 * Essentially invokes {@link #runTestClass(String, Editor)} 
	 * with the proper test class name. 
	 */
	public static void runFromMain(String... args) {
		Editor editor = Editor.code;// the default 
		switch (args.length) {
			case 0:
			...
			case 1:
				...
				if (!m.matches()) {
					throw new IllegalArgumentException(
							"Expected '-editor=(emacs|code)' but found '"
									+ args[0] + "'. ");
				}
				...
				break;
			default:
				throw new RuntimeException("Found more than one argument.");
		}
		runTestClass(new Throwable().getStackTrace()[1].getClassName(), editor);
		// TBD: use instead
